...
```
tcp_sent(tcp_pcb, Sent); // Called when a TCP ACK is received
tcp_recv(tcp_pcb, Recv); // Called when a TCP packet is received
tcp_err(tcp_pcb, Error); // Called when there is an error / disconnect event on the socket
```
...
# Connecting
With all the various gubbins initialised we can attempt a connection using the following code, the return of the function determines whether the _attempt_ to connect was successful, not whether the _actual_ connection itself was established.
...
[^6]: Github - pico-examples [link](https://github.com/raspberrypi/pico-examples/blob/master/pico_w/wifi/tcp_client/picow_tcp_client.c)
